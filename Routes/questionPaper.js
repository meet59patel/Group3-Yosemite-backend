...
router.get('/questionpaper/:questionPaperId', async (req,res, next) => {
    try {
        const questions = await Models.Question.find({ questionPaperID: req.params.questionPaperId})
...
router.get('/questionpaper/faculty/:facultyId', async(req,res,next) => {
    try{
        const questionPapers = await Models .QuestionPaper.find({ facultyID: req.params.facultyId }).sort({ createdAt: -1 })
...
router.post('/questionpaper/', async(req,res,next) => {
    try{
